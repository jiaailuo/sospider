...
class Cpi(IJob):
    ...
    def fetch_content_url(self):
        ...
        url = r'http://www.stats.gov.cn/tjsj/zxfb/index_1.html'
        res = requests.get(url, headers=headers)
        res.encoding = res.apparent_encoding
        soup = BeautifulSoup(res.text, 'lxml')
        items = soup.select('div.home div.main div.center div.center_list li')
        keyword = '{0}份居民消费价格'.format(datetime.strftime(self.bdate, '%Y年%m月'))
        for item in items:
